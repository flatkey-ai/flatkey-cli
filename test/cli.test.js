...
test("video generation falls back to task content endpoint when result omits url", async () => {
  const outDir = await mkdtemp(join(tmpdir(), "flatkey-video-content-"));
    ...
    fetch: async (url) => url.endsWith("/v1/video/generations")
      ...
      : url === "https://router.flatkey.ai/v1/videos/task_veo"
        ...
        : {
          ...
          status: 200,
          async arrayBuffer() {
            return Buffer.from("veo-video-bytes");
          },
        },
  });

  assert.equal(result.artifacts.length, 1);
  assert.equal(await readFile(result.artifacts[0].path, "utf8"), "veo-video-bytes");
});
